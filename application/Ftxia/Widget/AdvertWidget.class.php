    ...
    public function index($id) {
		...
		if(C('ftx_site_cache')){
			...
			if(false === $board_info = S($file)){
				$board_info = D('Adboard')->where($bmap)->find();
				S($file,$board_info);
			}
		}else{
			$board_info = D('Adboard')->where($bmap)->find();
		...
		if(C('ftx_site_cache')){
			...
			if(false === $ad_list = S($file)){
				$ad_list = M('Ad','cmf_','DB_DATAOKE')->field('id,type,name,url,content,desc,extimg,extval')->where($map)->order('ordid')->limit($limit)->select();
				S($file,$ad_list);
			}
		}else{
			$ad_list = M('Ad','cmf_','DB_DATAOKE')->field('id,type,name,url,content,desc,extimg,extval')->where($map)->order('ordid')->limit($limit)->select();

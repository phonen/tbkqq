...
class dapeiTag {
	...
	public function lists($options) {
		...
		$options['num'] = isset($options['num']) ? trim($options['num']) : '21';
		$options['order'] = isset($options['order']) ? trim($options['order']) : 'add_time desc';	
		if($options['cid']){			
		$id_arr = D('dapei_cate')->get_child_ids($options['cid'], true);
    ...
    public function cate($options) {
        ...
        if ($options['field'] != '*' || $options['where'] || $options['order'] != 'ordid') {
            $dapei_cate_mod = D('dapei_cate');
            $map = array('status'=>'1');
            $select = $dapei_cate_mod->field($options['field']); //字段
            ...
            if (false === $cate_list = F('artcate_list')) {
                $cate_list = D('dapei_cate')->cate_cache();
            }
            if ($options['cateid'] == 0) {

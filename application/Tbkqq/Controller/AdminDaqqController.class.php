...
class AdminDaqqController extends AdminbaseController {
	...
	public function item(){
		...
		$dataoke_model = M('TbkItem','cmf_','DB_DATAOKE');
		...
		if(IS_POST){
			if(isset($_POST['ids'])){
				...
				foreach($_POST['ids'] as $iid){
					...
					$taoke_model->where(array("iid"=>$iid))->save(array("item"=>$item,"img"=>$img));
				}
				$this->success("补采信息成功");
			}
		}
	...
	public function item_tuiguang(){
		if(IS_POST){
			...
			if(isset($_POST['ids'])){
				...
				foreach($_POST['ids'] as $iid){
					$item = array();
					$item = $item_model->where(array("iid"=>$iid))->find();

					if($item) continue;
					else {
						$item = M("TbkqqTaokeItemHistory")->where(array("iid"=>$iid))->find();
						if($item) continue;
						else{

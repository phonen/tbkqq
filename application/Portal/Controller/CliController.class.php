...
class CliController extends HomebaseController {
    ...
    public function get_dataoke_v1(){
        set_time_limit(0);
        $item_model = M('TbkqqItems');
        $item_model->where("source='dataoke'")->delete();
        $curtime = date("Y-m-d H:i:s");

        $cate_model = M("TbkqqCateCaiji");
        $cates = $cate_model->where("source = 'dataoke'")->select();
        $cate_data = array();
        foreach ($cates as $cate){
            $cate_data[$cate['cate']] = $cate['cate1'];
        }
        for($p=1;$p<=999;$p++) {
            $str = "";
...
//$iids[$item['GoodsID']]++;
                    ...
                    $data['cate_id'] = $cate_data[$item['Cid']];
                    ...
                    $data['coupon_end_time'] = strtotime($item['Quan_time']);
                    $data['quan_surplus'] = $item['Quan_surplus'];
                    $data['quan_receive'] = $item['Quan_receive'];
                    $data['quan_condition'] = $item['Quan_condition'];
                    $data['quanurl'] = "https://uland.taobao.com/cp/coupon?activityId=" .$item['Quan_id'] . "&itemId=" . $item['GoodsID'];
                    ...
                    //continue;
                    //else {
                    $dataoke_result = $item_model->add($data,$options=array(),$replace=true);
    ...
    public function  get_haodanku_v1()
    {
        set_time_limit(0);
        ...
        for ($p = 1; $p < 999; $p++) {
            ...
            if ($json) {
                foreach ($json as $item) {
                    ...
                    $data['commission'] = $item['tkrates'];

                    $urldata = get_url_data($item['couponurl']);
                    $data['sellerId'] = $urldata['sellerId'];
                    $data['quanurl'] = "https://uland.taobao.com/cp/coupon?activityId=" . $urldata['activityId'] . "&itemId=" . $item['itemid'];
                    $data['add_time'] = time();
                    if($item['cuntao'] == '1')$data['cun'] = '1';
    ...
    public function get_dataoke(){
        ...
        $dataoke_model->query("truncate table cmf_tbkqq_dataoke_item");
        //$iids = array();
...
//print_r($iids);
    ...
    public function check_quan(){
        ...
        $rs = M("TbkqqItems")->where(array("status"=>'underway'))->select();
        ...
        foreach ($rs as $val){
            ...
            if(!$coupon){
                ...
                if(!$coupon){
                    ...
                    M("CunItems")->where(array("num_iid"=>$val['num_iid']))->save(array("status"=>'-1'));
                }
    ...
    public function check_unuse_quan(){
        ...
        $rs = M("TbkqqItems")->where(array("status"=>'-1'))->select();
        ...
        foreach ($rs as $val){
            ...
            if(!$coupon){
                ...
                if(!$coupon){
                    echo "2222222222222222222222\n";
                    M("CunItems")->where(array("num_iid"=>$val['num_iid']))->delete();
                }
